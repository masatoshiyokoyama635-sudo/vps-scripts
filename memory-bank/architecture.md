...
- The script marks the feature with `Feature: http-basic-auth`; `install_shortcut()` only replaces a local shortcut script after downloading a syntax-valid remote script containing that marker, avoiding silent downgrade to a version that would ignore HTTP records. Temporary candidate configs retain a `.json` suffix because Xray infers the config format from the filename extension.
